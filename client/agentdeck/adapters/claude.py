...
class ClaudeCodeAdapter(AgentAdapter):
    ...
    async def run(self, prompt: str, cwd: Path, resume_session_id: str | None = None) -> AsyncIterator[AgentEvent]:
        ...
        command = [binary, "-p", "--output-format", "stream-json", "--verbose", "--permission-mode", "default"]
        if resume_session_id:
            command.extend(["--resume", resume_session_id])
        command.append(prompt)
        self._process = await asyncio.create_subprocess_exec(*command, cwd=cwd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)
