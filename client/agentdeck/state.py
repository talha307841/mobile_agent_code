...
class Outbox:
    def __init__(self, path: Path | None = None) -> None:
        ...
        with self._connect() as db:
            db.execute(
                "CREATE TABLE IF NOT EXISTS outbox (id TEXT PRIMARY KEY, body TEXT NOT NULL, created_at TEXT DEFAULT CURRENT_TIMESTAMP)"
            )
    ...
    def add(self, message: Envelope) -> None:
        with self._connect() as db:
            db.execute(
                "INSERT OR IGNORE INTO outbox(id, body) VALUES (?, ?)",
                (str(message.id), message.model_dump_json()),
            )
