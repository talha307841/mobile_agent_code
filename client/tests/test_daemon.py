...
from agentdeck.daemon import Daemon
...
async def test_dangerous_task_requires_approval(tmp_path):
    ...
    daemon.send = capture
    task_id = str(uuid4())
    await daemon.handle(Envelope(type=MessageType.TASK_START, payload={"task_id": task_id, "repository_id": str(repo.id), "repository_path": str(repo_path), "prompt": "deploy this to production", "agent": "mock"}))
