...
async def device_socket(socket: WebSocket) -> None:
    ...
    raw = authorization.removeprefix("Bearer ") or socket.query_params.get("token", "")
    async with SessionLocal() as db:
        device = await db.scalar(
            select(Device).where(
                Device.credential_hash == hash_token(raw), Device.revoked.is_(False)
            )
        )
        if device is None:
            await reject(socket)
    ...
    await hub.notify_user(
        user_id,
        Envelope(type=MessageType.STATUS, payload={"device_id": str(device_id), "online": True}),
    )
    try:
        while True:
        ...
        await hub.notify_user(
            user_id,
            Envelope(
                type=MessageType.STATUS, payload={"device_id": str(device_id), "online": False}
            ),
        )
...
async def _handle_device_message(device_id: UUID, user_id: UUID, envelope: Envelope) -> None:
    async with SessionLocal() as db:
        ...
        if envelope.type == MessageType.HELLO:
            ...
            existing = list(
                (
                    await db.scalars(select(Repository).where(Repository.device_id == device_id))
                ).all()
            )
            ...
            for item in reported:
                ...
                repo = await db.scalar(
                    select(Repository).where(
                        Repository.id == repo_id, Repository.device_id == device_id
                    )
                )
                if repo:
                    ...
                    db.add(
                        Repository(
                            id=repo_id, device_id=device_id, name=item["name"], path=item["path"]
                        )
                    )
            await db.commit()
        ...
        task = await db.scalar(
            select(Task).where(
                Task.id == UUID(task_id_raw), Task.device_id == device_id, Task.user_id == user_id
            )
        )
        if task is None:
            return
        if envelope.type == MessageType.TASK_EVENT:
            ...
            exists = await db.scalar(
                select(TaskLog.id).where(TaskLog.task_id == task.id, TaskLog.sequence == sequence)
            )
            if not exists:
                db.add(
                    TaskLog(
                        task_id=task.id,
                        sequence=sequence,
                        stream=envelope.payload.get("stream", "system"),
                        text=envelope.payload.get("text", "")[:1_000_000],
                        payload=envelope.payload.get("raw") or {},
                    )
                )
            ...
            task.state = (
                TaskState(state) if state in TaskState._value2member_map_ else TaskState.FAILED
            )
            ...
            approval = Approval(
                user_id=user_id,
                task_id=task.id,
                action=envelope.payload.get("action", "unknown"),
                details=envelope.payload.get("details") or {},
            )
            db.add(approval)
            await db.flush()
            ...
            record_audit(
                ...
                user_id=user_id,
                device_id=device_id,
                details={"task_id": str(task.id), "action": approval.action},
            )
        record_audit(
            db,
            f"device.{envelope.type.value}",
            user_id=user_id,
            device_id=device_id,
...
async def mobile_socket(socket: WebSocket) -> None:
    authorization = socket.headers.get("authorization", "")
    token = authorization.removeprefix("Bearer ") or socket.query_params.get("token", "")
    try:
        ...
        while True:
            ...
            if envelope.type == MessageType.HEARTBEAT:
                await socket.send_text(
                    Envelope(type=MessageType.HEARTBEAT_ACK, reply_to=envelope.id).model_dump_json()
                )
    except (WebSocketDisconnect, ValueError):
        pass
    finally:
